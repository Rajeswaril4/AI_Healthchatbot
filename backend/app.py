...
#   FRONTEND_URL  = https://your-frontend.onrender.com  (or Vercel/Netlify URL)
...
# ==================== DATABASE ====================
# Render injects DATABASE_URL automatically for PostgreSQL add-ons.
# Render still uses the old "postgres://" scheme; SQLAlchemy needs "postgresql://"
...
# ==================== CORS ====================
# Pull extra allowed origins from env (comma-separated) for Render / custom domains
_extra_origins = [
...
def create_default_admin():
    ...
    admin = User(email=admin_email, username=admin_username, role="admin", is_active=True)
...
The AI HealthBot Team
        ...
        # Use FRONTEND_URL for the CTA button so it works in production
        html_content = f"""
<html>
    ...
    <div style="max-width: 600px; margin: 0 auto; padding: 0;">
      ...
      <div style="background: #ffffff; padding: 40px 30px;">
        ...
        <p style="font-size: 16px; color: #333; line-height: 1.8;">
          Thank you for registering with <strong>AI HealthBot</strong>!
        </p>

        <div style="background: #f8f9fa; padding: 25px; border-radius: 8px; margin: 25px 0;">
          <h3 style="color: #667eea; margin-top: 0; font-size: 18px;">What you can do with AI HealthBot:</h3>
          <table style="width: 100%; border-collapse: collapse;">
            <tr><td style="padding: 8px 0; font-size: 15px;">✅ AI-powered disease predictions</td></tr>
            <tr><td style="padding: 8px 0; font-size: 15px;">✅ Personalized specialist recommendations</td></tr>
            <tr><td style="padding: 8px 0; font-size: 15px;">✅ Track your prediction history</td></tr>
            <tr><td style="padding: 8px 0; font-size: 15px;">✅ Find nearby healthcare facilities</td></tr>
          </table>
        </div>

        <div style="text-align: center; margin: 35px 0;">
          <a href="{FRONTEND_URL}" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 14px 40px; text-decoration: none; border-radius: 30px; display: inline-block; font-weight: bold; font-size: 16px;">
            Get Started Now →
          </a>
        </div>
        ...
        <div style="background: #fff3cd; border-left: 4px solid #ffc107; padding: 20px; margin-top: 30px; border-radius: 4px;">
          <p style="margin: 0; font-size: 14px; color: #856404;">
            <strong>⚠️ Important:</strong> AI HealthBot is a diagnostic aid — always consult qualified healthcare professionals for accurate diagnosis and treatment.
          </p>
        </div>

        <div style="margin-top: 40px; padding-top: 30px; border-top: 1px solid #e9ecef; text-align: center;">
          <p style="color: #667eea; font-weight: bold; font-size: 15px; margin: 15px 0 0 0;">The AI HealthBot Team</p>
        </div>
      </div>

      <div style="background: #f8f9fa; padding: 20px 30px; text-align: center;">
        <p style="color: #6c757d; font-size: 12px; margin: 5px 0;">© 2024 AI HealthBot. All rights reserved.</p>
      </div>
    </div>
  </body>
</html>
...
try:
    ...
    print("⚠️  Error loading artifacts:", e)

if not symptom_columns:
    symptom_columns = ["fever", "cough", "headache", "fatigue", "sore_throat"]
if not disease_info:
    disease_info = {"common cold": {"description": "A mild viral infection.", "precautions": ["Rest", "Stay hydrated"]}}
if model is None:
...
def google_login():
    try:
        ...
        # BACKEND_URL drives the callback — works both locally and on Render
        callback_url = os.getenv(
            "GOOGLE_CALLBACK_URL",
            f"{BACKEND_URL}/api/auth/google/callback"
        )

        print(f"🔑 Starting Google OAuth — callback: {callback_url}")
...
def google_callback():
    try:
        ...
        print(f"❌ Exception in Google callback: {e}")
        import traceback; traceback.print_exc()
...
def get_history():
    try:
        ...
        return jsonify({'ok': True, 'history': history_payload, 'count': len(history_payload)}), 200
...
def predict():
    try:
        ...
        selected_symptoms = body.get('symptoms', [])
        ...
        try:
            if hasattr(model, "predict_proba"):
                ...
                confidence_value = float(probs[int(match_idx[0])]) if match_idx.size else float(probs.max())
        except Exception as ce:
            print("⚠️  Confidence error:", ce)
...
def admin_dashboard(admin_user):
    try:
        ...
        recent_reg         = User.query.filter(User.created_at >= thirty_days_ago).count()
        recent_pred        = Prediction.query.filter(Prediction.created_at >= seven_days_ago).count()
        ...
        return jsonify({
            ...
            "stats": {
                ...
                "recent_predictions":   recent_pred,
...
def admin_get_users(admin_user):
    try:
        ...
        if search:
            pattern = f"%{search}%"
            q = q.filter((User.email.like(pattern)) | (User.username.like(pattern)))
        if role_filter:
...
def admin_update_user_role(admin_user, user_id):
    try:
        ...
        log_admin_action(admin_user.id, "updated_user_role", "user", user_id,
                         {"old_role": old_role, "new_role": new_role})

        return jsonify({"ok": True, "message": "User role updated", "user": user.to_admin_dict()}), 200
...
def admin_delete_user(admin_user, user_id):
    try:
        ...
        log_admin_action(admin_user.id, "deleted_user", "user", user_id, {"email": user_email})
...
def admin_delete_prediction(admin_user, prediction_id):
    try:
        ...
        log_admin_action(admin_user.id, "deleted_prediction", "prediction", prediction_id,
                         {"user_id": user_id, "disease": disease})
...
def query_overpass(query_text: str, timeout_seconds: int = 60):
    ...
    for url in OVERPASS_ENDPOINTS:
        try:
            ...
            print(f"⚠️  Overpass error at {url}:", e)
    raise RuntimeError("All Overpass endpoints failed")


def get_cache_key(lat: float, lng: float, radius: int, specialist: str) -> str:
    return f"nearby:{lat:.5f}:{lng:.5f}:{radius}:{normalize_name(specialist)}"
...
def query_overpass_with_cache(query_text: str, cache_key: str, timeout_seconds: int = 60):
    ...
    if cached:
        ...
        if now - cached_at < _OVERPASS_CACHE_TTL_SEC:
            return payload
...
def _build_overpass_query(lat: float, lon: float, radius: int, specialist_token: str):
    ...
    tok_esc = tok.replace('"', '\\"')
    q_parts = [
        f'node(around:{radius},{lat},{lon})[amenity~"hospital|clinic"];',
        f'way(around:{radius},{lat},{lon})[amenity~"hospital|clinic"];',
        f'node(around:{radius},{lat},{lon})[healthcare];',
        f'way(around:{radius},{lat},{lon})[healthcare];',
    ]
    if tok_esc:
...
def nearby():
    try:
        ...
        try:
            result = query_overpass_with_cache(q, cache_key, timeout_seconds=30)
        except Exception as e:
            print(f"❌ Overpass error: {e}")
            return jsonify({
                "ok": True, "count": 0, "places": [],
        ...
        for el in result.get("elements", []):
            ...
            addr_parts = [tags[k] for k in ("addr:housenumber", "addr:street", "addr:city") if tags.get(k)]
...
# ==================== DEBUG ENDPOINTS (disable in prod) ====================
_DEBUG_ENABLED = os.getenv("ENABLE_DEBUG_ROUTES", "false").lower() in ("1", "true", "yes")
...
if _DEBUG_ENABLED:
    ...
    def debug_predictions():
        try:
            ...
            return jsonify({'total': len(all_preds), 'predictions': [
                {'id': p.id, 'user_id': p.user_id, 'disease': p.disease,
    ...
    def debug_check_history():
        try:
            ...
            result = {'total_predictions': len(all_predictions), 'predictions_by_user': {}}
            for pred in all_predictions:
                uid = pred.user_id or 'None'
                result['predictions_by_user'].setdefault(uid, []).append({
                    'id': pred.id, 'disease': pred.disease,
